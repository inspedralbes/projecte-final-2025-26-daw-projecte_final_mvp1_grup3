...
class MissionService
{
    ...
    private function compleixMissio(MissioDiaria $missio, int $userId, int $habitId, Carbon $timestamp): bool
    {
        ...
        $avui = $this->diaActivitat($timestamp);
        ...
        // D. hab_dificultat: existeix registre avui amb dificultat X (facil / media / dificil)
        if ($tipus === 'hab_dificultat') {
            $dificultat = (string) ($parametres['dificultat'] ?? '');
    ...
    public function obtenirProgresMissio(int $userId): ?array
    {
        ...
        $avui = $this->diaActivitat(Carbon::now());
        $tipus = $missio->tipus_comprovacio ?? '';
    ...
    /**
     * Normalitza el dia d'activitat segons el fus horari de l'aplicació.
     */
    private function diaActivitat(Carbon $timestamp): Carbon
    {
        $timezone = config('app.timezone', 'Europe/Madrid');

        return $timestamp->copy()->setTimezone($timezone)->startOfDay();
    }
    ...
    {
        $variants = DificultatNormalizer::valorsBdEquivalent($dificultat);
        if ($variants === []) {
            return false;
        }
        ...
        return RegistreActivitat::whereHas('habit', function ($q) use ($userId, $variants) {
            $q->where('usuari_id', $userId)
                ->whereIn(DB::raw('LOWER(COALESCE(dificultat, \'\'))'), $variants);
    ...
    private function existeixRegistreAvuiAmbDificultats(int $userId, Carbon $avui, array $dificultats): bool
    {
        ...
        foreach ($dificultats as $dificultat) {
            foreach (DificultatNormalizer::valorsBdEquivalent((string) $dificultat) as $variant) {
                $variants[] = $variant;
            }
        }
