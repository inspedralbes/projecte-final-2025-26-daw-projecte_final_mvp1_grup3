 ...
 * Manté les claus al backend i retorna respostes normalitzades.
 */
class ExternalApiProxyService
{
     ...
     *
     * Si la query coincideix amb una categoria muscular → filtra per categoria.
     * Sinó → cerca per nom a través de exerciseinfo de múltiples categories.
     *
     * @return array<string, mixed>
     */
    ...
    /**
     * Obté el detall complet d'un exercici de Wger per ID.
     * Extreu nom, categoria, músculs, equipament i descripció.
     *
     * @return array<string, mixed>
     */
    public function obtenirDetallExercici(string $exerciseId): array
    {
        ...
        try {
            ...
            if (!is_array($data)) {
                return [
                    'ok'    => false,
                    'error' => 'Resposta invàlida de Wger',
                ];
            }
     ...
     * Endpoint ràpid, fiable i sense clau d'API.
     *
     * @return array<string, mixed>
     */
    ...
    /**
     * Consulta context de clima per hàbits de la llar.
     ...
     * Servei gratuït, sense clau d'API.
     */
