			...
			<div class="grid grid-cols-2 gap-12 items-stretch px-8 py-8 bg-white rounded-2xl shadow-sm">
				...
				<div class="bg-white rounded-xl p-8 shadow-md" style="max-width:420px">
					<div class="flex flex-col items-center gap-4">
						...
						<h2 class="text-2xl font-semibold text-gray-800">Uneix-te a Loopy</h2>
						<p class="text-sm text-gray-500 text-center">Crea el teu compte i comença el teu viatge.</p>
					</div>

					<form class="mt-6 space-y-4">
						<div>
							<label class="block text-xs font-medium text-gray-600 mb-2">NOM</label>
							<input type="text" placeholder="El teu nom" class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-200" />
						</div>
						<div>
							<label class="block text-xs font-medium text-gray-600 mb-2">EMAIL</label>
							<input type="email" placeholder="el.teu.email@exemple.com" class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-200" />
						</div>
						<div>
							<label class="block text-xs font-medium text-gray-600 mb-2">CONTRASSENYA</label>
						...
						<div>
							<label class="block text-xs font-medium text-gray-600 mb-2">CONFIRMAR CONTRASSENYA</label>
							<input type="password" placeholder="••••••••" class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-200" />
						</div>

						<div class="pt-2">
							<button type="button" class="w-full bg-green-600 hover:bg-green-700 text-white font-medium py-3 rounded-lg">REGISTRAR-SE</button>
						</div>

						<div class="pt-2">
							<NuxtLink to="/login">
								<button type="button" class="w-full bg-white border border-gray-200 text-gray-700 font-medium py-3 rounded-lg hover:bg-gray-50">TORNAR AL LOGIN</button>
							</NuxtLink>
						</div>
					</form>
				</div>
				...
				<div class="grid grid-cols-2 gap-4 h-full">
					...
					<template v-if="categoriaSeleccionada === null">
						<div class="col-span-2 bg-gradient-to-br from-blue-50 to-blue-100 rounded-xl p-4 shadow-sm flex flex-col items-center justify-center">
							<div class="text-xs font-bold text-gray-600 text-center">En quina àrea et vols centrar?</div>
						</div>
						<div class="bg-white rounded-xl p-2 shadow-sm flex items-center justify-center cursor-pointer hover:bg-blue-50" @click="seleccionarCategoria('gym')">
							<div class="text-xs font-medium text-gray-700">💪 Activitat física</div>
						</div>
						<div class="bg-white rounded-xl p-2 shadow-sm flex items-center justify-center cursor-pointer hover:bg-green-50" @click="seleccionarCategoria('nutrition')">
							<div class="text-xs font-medium text-gray-700">🥗 Alimentació</div>
						</div>
						<div class="bg-white rounded-xl p-2 shadow-sm flex items-center justify-center cursor-pointer hover:bg-yellow-50" @click="seleccionarCategoria('study')">
							<div class="text-xs font-medium text-gray-700">📚 Estudi</div>
						</div>
						<div class="bg-white rounded-xl p-2 shadow-sm flex items-center justify-center cursor-pointer hover:bg-purple-50" @click="seleccionarCategoria('reading')">
							<div class="text-xs font-medium text-gray-700">📖 Lectura</div>
						</div>
						<div class="bg-white rounded-xl p-2 shadow-sm flex items-center justify-center cursor-pointer hover:bg-pink-50" @click="seleccionarCategoria('wellness')">
							<div class="text-xs font-medium text-gray-700">🧘 Benestar</div>
						</div>
						<div class="bg-white rounded-xl p-2 shadow-sm flex items-center justify-center cursor-pointer hover:bg-red-50" @click="seleccionarCategoria('smoking')">
							<div class="text-xs font-medium text-gray-700">🚭 Vida sense Fum</div>
						</div>
						<div class="bg-white rounded-xl p-2 shadow-sm flex items-center justify-center cursor-pointer hover:bg-indigo-50" @click="seleccionarCategoria('cleaning')">
							<div class="text-xs font-medium text-gray-700">🏠 Neteja Express</div>
						</div>
						<div class="bg-white rounded-xl p-2 shadow-sm flex items-center justify-center cursor-pointer hover:bg-gray-50" @click="seleccionarCategoria('hobby')">
					...
					<template v-else>
						<div v-for="(pregunta, index) in obtenirPreguntesActuals()" :key="pregunta.id" class="bg-white rounded-xl p-3 shadow-sm flex flex-col items-center justify-center overflow-hidden">
							<div class="text-xs font-bold text-gray-600 text-center mb-2">Pregunta {{ index + 1 }}/{{ obtenirPreguntesActuals().length }}</div>
						...
						<div class="col-span-full mt-2 grid grid-cols-2 gap-2">
							<button @click="seleccionarCategoria(null)" class="w-full bg-gray-300 hover:bg-gray-400 text-gray-800 font-medium py-2 rounded-lg text-xs">TORNAR</button>
    ...
    seleccionarCategoria: async function(categoria) {
      ...
      if (categoria) {
        ...
        try {
          ...
          resultat = await useFetch("http://localhost:8000/api/preguntes-registre/" + idCategoria);
