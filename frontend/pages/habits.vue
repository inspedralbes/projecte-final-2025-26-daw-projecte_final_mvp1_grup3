        ...
        <div class="lg:col-span-2 space-y-6">
          ...
          <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            <div class="flex items-center gap-3 mb-4">
              ...
              <h2 class="text-lg font-bold text-gray-800">Detalles del Hábito</h2>
            </div>
            ...
            <div class="space-y-4">
              ...
              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Motivación (Opcional)</label>
                <textarea v-model="form.motivation" placeholder="¿Por qué quieres empezar este hábito?" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white transition-all resize-none h-24"></textarea>
              </div>

              <div>
                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Icono Rápido</label>
                <div class="flex gap-3">
                  <button v-for="icon in icons" :key="icon" @click="selectIcon(icon)" :class="{'bg-green-500 text-white': form.icon === icon, 'bg-gray-100 text-gray-600 hover:bg-gray-200': form.icon !== icon}" class="w-10 h-10 rounded-full flex items-center justify-center transition-colors">
                    {{ icon }}
                  </button>
                </div>
              </div>
          ...
          <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <button v-for="cat in categories" :key="cat.id" @click="selectCategory(cat.id)" :class="{'ring-2 ring-green-500 bg-green-50': form.category === cat.id, 'bg-white border border-gray-200 hover:border-green-300': form.category !== cat.id}" class="p-4 rounded-xl flex flex-col items-center justify-center gap-2 transition-all">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div class="space-y-4">
                ...
                <div>
                  ...
                  <input v-model="form.reminder" type="time" class="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm w-full focus:outline-none focus:ring-2 focus:ring-green-500" />
                </div>
              </div>
            </div>
            ...
            <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <p class="text-sm text-gray-500 mb-4">Elige el estilo visual de tu tarjeta de hábito.</p>
              
              <div class="flex gap-3 mb-6">
                <button v-for="color in colors" :key="color" @click="form.color = color" :style="{ backgroundColor: color }" class="w-10 h-10 rounded-full transition-transform hover:scale-110 focus:outline-none ring-2 ring-offset-2" :class="{'ring-gray-400': form.color === color, 'ring-transparent': form.color !== color}"></button>
              </div>

              <!-- Preview Card -->
              <div class="bg-gray-50 rounded-xl p-4 flex items-center gap-4 opacity-75">
                <div :style="{ backgroundColor: form.color || '#10B981', color: 'white' }" class="w-10 h-10 rounded-lg flex items-center justify-center text-lg">
                   {{ form.icon || '📝' }}
                </div>
          ...
          <!-- Submit Button -->
          <button @click="createHabit" class="w-full bg-green-700 hover:bg-green-800 text-white font-bold py-4 rounded-xl shadow-lg shadow-green-700/20 transition-all transform active:scale-95 flex items-center justify-center gap-2">
        ...
        <div class="lg:col-span-1">
          <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 h-full">
            ...
            <div v-else class="space-y-4">
              <div v-for="habit in habitStore.habits" :key="habit.id" class="flex items-center gap-4 p-4 rounded-xl bg-gray-50 hover:bg-white hover:shadow-md transition-all border border-transparent hover:border-gray-100 group">
